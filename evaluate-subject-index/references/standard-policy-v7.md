...
An unresolved complete-path-fit set similarly blocks full migration. A hash-bound `subject-index-v7-locator-fit-supplement-v2` may carry separately authorized decisions for every and only that set. Each new decision supplies an approved category plus either a concise public-safe rationale or a reference to a validated hash-bound rationale ledger. The calculator consumes only the category and derives frozen `F`, `L=min(T,F)`, and grade values; rationale is never a scoring input. Conflict adjudication resolves the prospective V7 fit axis without declaring a historical classifier correct. Supplemental decisions are per-view, applied in memory, and cannot modify historical evidence or non-fit state. Invalid states are excluded from the set and remain ineligible. Historical V1 supplements remain readable through compatibility validation.
