...
Tooling patch `dimension-score-cli-v7.0.5` aligns the V7 locator-fit preflight schema with that V7.0.4 implementation by permitting a conflict-routed unresolved record to preserve `treatment_class: "absent"`. The schema limits this newly admitted unresolved form to `legacy_structured_fit_classification_conflict_requires_adjudication`; it does not admit `unavailable`, which remains legally tied to `uninspectable` judgment and deterministic bounded treatment. The exact-scope supplement still supplies fit independently, while the unchanged calculation applies `L=min(T,F)` and therefore retains zero combined credit when frozen treatment is absent. No treatment or fit value, mapping, formula, dimension, gate, cap, threshold, rubric identity, calculation-profile identity, or historical artifact contract changes. V7.0.3 and V7.0.4 migration records retain their prior validation requirements.
