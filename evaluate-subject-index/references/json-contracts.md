...
| V6 score migration | `score-migration-v5-to-v6.schema.json` | Immutable V5 artifact bytes and hashes, frozen-evidence checks, old/new score and precision comparison, unchanged gates/selectivity, and calculation-only invalidation |
| V6 projection metadata | `v6-projection-metadata.schema.json` | Customer labels, gates, display content, comparison identities, and optional migration binding used to build V6 projections |
...
V6 calculation V2 stores exact decimal base and post-cap values as strings, final half-step ratings and two-decimal points as JSON numbers, and every dimension's bound input artifacts, denominators, exclusion reasons, mappings, components, caps, endpoints, rounding, rating, weight, and points. Page-reference Reliability additionally retains every locator credit and disqualifier plus both precision measures. A migrated V2 calculation carries a hash-covered `migration_context` that resolves its V5-to-V6 record and exact historical V5 calculation, result, and web-report hashes. Projection validation resolves and hash-checks those immutable historical artifacts, reruns the V5 projection validator, checks frozen input lineage and evidence identity, preserves representation-correction provenance through the unchanged historical projections, requires gate arrays and hashes to match exactly, and reconstructs the V6 result and web comparison. Native V6 projections are validated through the same calculation, item, precision, scorecard, and gate reconstruction path without migration fields.
