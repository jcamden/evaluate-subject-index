...
Required: one candidate ID, restricted candidate PDF, candidate-evaluation repository, benchmark repository, validated source state, edition identity, expanded page map, chunk manifest, frozen judgment-policy identity, and the version-one preparation-compatibility marker. That marker remains `subject-index-rubric-v4`; it is not the active V6 score identity. Optional: immutable benchmark preparation ref and adapter (`auto`, `generic-pdf-layout`, or `indexerlabs-two-column`). Resolve and record the benchmark default-branch head once when no ref is supplied.
...
Run `scripts/item_grade_cli.py build-assessments --grading-policy subject-index-item-grading-v2` and produce `subject-index-item-assessments-v3` under [item-grading.md](item-grading.md). In full mode, require one judgment for every resolved locator, heading node, cross-reference, path, and expected source subject. In pilot mode, retain unsampled inventory items with neutral `not_measured` grades. The helper first creates a projection-safe V2 evidence binding, then applies the V6 locator grades 100/70/25/0/neutral and recalculates complete-path diagnostics. Every item carries its grade/color, factors, public-safe popover, confidence where applicable, and evidence IDs. The five assessment arrays and summaries must exhaust the corresponding inventory or expected-subject ID sets without duplicates. Register the complete artifact as private by default because its labels and locators may reproduce the candidate; publishing a full colored index requires a separate rights and disclosure decision.
...
```text
python scripts/dimension_score_v6_cli.py preflight --input dimension-calculation-input.json
python scripts/dimension_score_v6_cli.py calculate --input dimension-calculation-input.json --output dimension-calculations.v6.json
```

The V6 helper validates every referenced schema and hash, reconstructs the exact audit sets, rejects inconsistent combined locator states, and derives all six ratings using `Decimal` with `ROUND_HALF_UP`. Page-reference Reliability records every locator credit, both strict and weighted precision, treatment recall, weighted F1, caps, uncertainty endpoints, rounding, and points. All other formulas and safeguards delegate to the unchanged V5 implementations. Full mode stops on any required `not_measured` item. A numeric dimension is published only when both bounds have the same public rating and applied-cap identity.

Create `subject-index-v6-projection-metadata-v1`, then use `dimension_score_v6_cli.py build-projections` to produce `subject-index-evaluation-result-v7` and `subject-index-web-report-v5`. Both bind the exact V2 calculation and V3 item artifacts. Publication-readiness gates are copied as independent claim restrictions and never enter score arithmetic. Diagnostic grades remain non-additive. Qualitative anchors may warn after calculation but cannot select or override a number.

After creating the result and web report, validate both projections against the exact calculation bytes:

```text
python scripts/dimension_score_v6_cli.py build-projections \
  --calculation dimension-calculations.v6.json \
  --item-assessments item-assessments.v3.json \
  --metadata v6-projection-metadata.json \
  --evaluation-result-output evaluation-result.v7.json \
  --web-report-output web-report.v5.json

python scripts/dimension_score_v6_cli.py validate-projections \
  --calculation dimension-calculations.v6.json \
  --evaluation-result evaluation-result.v7.json \
...
For a V5-to-V6 score-only migration, pass the bound migration record to `build-projections`. The migration and projection validators preserve and hash-check the V5 calculation, result, item binding, web report, gates, and frozen ledgers while creating separately versioned V6 projections. Current tooling derives one common ancestor-only `artifact_path_root`, stores every V5/V6 binding as a normalized descendant without `..`, and rejects resolved or symlink paths outside that root. This permits separate sibling `v5-migration/` and `v6-migration/` directories while preserving containment.

## `preflight-v6-scoring`

Run `dimension_score_v6_cli.py preflight` against frozen ledgers before a native V6 calculation. It inherits every V5 integrity/sufficiency check and additionally requires each locator's explicit, logically consistent judgment, treatment class, source-scope status, error codes/structured defects, and inspectability state. It never mutates evidence or infers missing fields from prose.

## `migrate-score-only`

For current V5-to-V6 migration, require the exact frozen calculation input and immutable V5 calculation, result, item binding, and web report. Run `dimension_score_v6_cli.py migration-preflight`, then `score-only-migration` with separate V6 calculation and migration-record outputs, the exact methodology commit, and an explicit RFC 3339 UTC timestamp. V5 and V6 artifacts may occupy sibling version directories; `dimension-score-cli-v6.0.3` records their common bounded ancestor and descendant-only bindings deterministically. Use `item_grade_cli.py upgrade-v6-assessments` with the immutable V2 item artifact and its exact locator/structure evidence to create V3 diagnostics. The preflight reports missing locator fields precisely and stops without source re-entry or prose inference. The operation preserves historical bytes, frozen evidence, representation provenance, strict precision, selectivity, and gates; it invalidates only active calculation-derived projections and emits `subject-index-score-migration-v5-to-v6-v1`. See [score-migration-v5-to-v6.md](score-migration-v5-to-v6.md).

Historical V4-to-V5 migration remains available through `dimension_score_cli.py` and [score-migration-v4-to-v5.md](score-migration-v4-to-v5.md). Never use a V6 command to reinterpret or overwrite V4/V5 history.

## `build-web-report`

Required: valid evaluation result and selected representative examples.

Create `subject-index-web-report-v5` using the V6 projection builder and public presentation strategy in [customer-methodology.md](customer-methodology.md). Organize the report in four layers: (1) overall result and publication-readiness status, (2) six plain-language quality questions, (3) measured evidence and representative examples, and (4) complete methodology, scope, and limitations. Make layers three and four expandable in the web interface.

Include both weighted locator precision and strict substantive precision, full reconstructable Page-reference Reliability provenance, density, gates, strengths, defects, examples, comparability, disclosure, and limitations. Label the two precision measures distinctly and explain that weak presence remains editorially unjustified and earns zero selectivity credit. Every report includes `migration_comparison` (`not_applicable` for native V6) and explicit score views. A migrated report exposes V5/V6 totals, all dimension changes, the precision comparison, methodology commit, and unchanged gate hashes. Reference the exact V3 item artifact and render grade-25 explanations under [item-grading.md](item-grading.md). Keep gates outside arithmetic and use public-safe evidence IDs. See [web-report-fields-v5.md](web-report-fields-v5.md).
