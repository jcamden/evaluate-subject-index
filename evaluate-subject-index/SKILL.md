...
9. Derive all six V5 ratings from validated raw ledger statuses with `subject-index-dimension-calculation-v1`. Never submit diagnostic item grades, gates, or manually selected headline ratings as calculation inputs.
...
Use `scripts/state_cli.py` for deterministic state initialization, status, dependency-aware next-step selection, state transitions, artifact registration, hashing, manifest updates, score-profile-only invalidation, and validation. Use `scripts/policy_cli.py` to instantiate and hash the score-independent judgment policy. Use `scripts/bundle_cli.py` for portable/private checkpoints, exports, artifact inventories, and safe imports. Use `scripts/page_chunk_cli.py` to expand page-label maps, validate user-approved chunk ranges, split source PDFs, and create locator-only chunk packets. Use `scripts/benchmark_review_cli.py` to enumerate benchmark-review denominators and enforce full-review and final-freeze gates. Use `scripts/item_grade_cli.py` to create stable display-item identities and deterministic diagnostic grades. Use `scripts/dimension_score_cli.py` for V5 sufficiency preflight, the six deterministic calculations, and score-only migration. `scripts/score_cli.py scorecard` is historical V4 validation only; its density helper remains available for V4 artifacts. Do not ask the language model to maintain arithmetic, item-grade aggregation, or workflow state when a script can do it.
...
Candidate preparation may begin after the source SHA-256, edition identity, expanded page map, chunk manifest, policy profile/hash, legacy preparation-compatibility marker, and audit mode are frozen. That version-one marker remains `subject-index-rubric-v4` and is not the active V5 score identity. Preparation does not require a synthesized, reviewed, or frozen benchmark. Run it in an isolated worker context that does not expose candidate material to any source-discovery, synthesis, or benchmark-review context. Normalized candidate data is never benchmark evidence, and a publisher index is never ground truth.
...
At `score-index`, first create the separate diagnostic `subject-index-item-assessments-v2`. It must bind the exact item-inventory artifact and complete calculation evidence identity and exhaust the locator, path, node, cross-reference, and expected source-subject ID sets exactly once. Build `subject-index-dimension-calculation-input-v1` using the frozen audit mode plus exact paths and SHA-256 hashes for the canonical user-approved chunk manifest and every scoring ledger; run `dimension_score_cli.py preflight`, then—for a new `structure-audit-v4` only—`calculate` to create `subject-index-dimension-calculations-v1`. Historical V3 inputs must use `score-only-migration`; the ordinary calculation path rejects them so the exact V4 result, migration record, and gates cannot be bypassed. Preflight must prove the manifest's complete chunk set exactly once, uniform upstream identities, exact locator- and missing-access-audit set hashes, and matching frozen audit mode. Every calculation component must expose its raw numerator and denominator separately from its normalized value, weight, cap effects, and rounding. Bind the calculation artifact and the exact V2 item-assessment path/hash in a new `subject-index-evaluation-result-v6`; the web report must bind that same item-assessment file. Never submit item grades, gates, or manually selected ratings. Preserve gates as separate claim restrictions and expose the calculation details, required migration comparison, and explicit observed/counterfactual score views in `subject-index-web-report-v4`; run `validate-projections` against both projections before publication.

For a historical V4 evaluation, run the same no-mutation preflight and then `score-only-migration` with the exact methodology commit and an explicit UTC timestamp. Preserve the V4 result, prove ledger-byte invariance, and write `subject-index-score-migration-v2` with portable relative paths, old/new totals and six-dimension scorecards, complete input lineage, and unchanged gate arrays. Preserve V1 migration records as readable history. Retain identical gate outcomes only after every gate-bearing policy and evidence identity matches. Record any representation correction only through immutable provenance arguments and keep its adjusted calculations as counterfactual score views; never attribute their difference to V5. Bind the exact migration record in the V6 result, copy the historical critical gates, and use `validate-projections` with an output path, the same methodology commit, and a validation timestamp to produce `subject-index-score-migration-validation-v1`. That receipt must hash-bind the old result, calculation, migration, new result, web report, score views, and unchanged gates. A public export whose audit-set identity differs from the canonical checkpoint cannot be substituted. Adopt the V5 score identity with `state_cli.py set-score-calculation-profile --preflight ... --calculation-input ...` only after the command independently reproduces a sufficient preflight and proves every supplied evidence file is the exact frozen, required artifact registered in canonical state and its manifest; this resets `scoring` and `web_report` only. Read [score-migration-v4-to-v5.md](references/score-migration-v4-to-v5.md).
