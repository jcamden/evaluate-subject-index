...
Create item grades as a separate diagnostic layer under [item-grading-v3.md](references/item-grading-v3.md). A locator grade equals exactly 100 times `min(T,F)`; never average grades into Page-reference Reliability. Grade every other item family under its preserved policy and keep pilot `not_measured` records neutral. Under `ITEM-PROJECTION-DEFECT-ID-ASC-V1`, arrays derived from applicable defect identity sets deduplicate by `defect_id` and use ascending `defect_id` order; do not reorder heading, node, locator, reference, display, or source-evidence arrays whose existing order is intentional.
...
At `score-index`, run `dimension_score_v7_cli.py preflight`, derive `subject-index-structure-locator-review-v1`, and run `calculate` to create `subject-index-dimension-calculations-v4` under the unchanged calculation profile V3. V7 derives independent page-treatment and complete-path-fit ceilings exclusively from frozen judgment, treatment class, scope/inspectability, error codes, applicable structured defects, and severity. It combines them with `min(T,F)`, never multiplication or rationale prose. New locator work uses `locator-audit-v2`: preserve one nonempty locator-specific public-safe `evidence_summary`, and add `fit_rationale` only when axes diverge, fit is nonperfect, structured classifiers conflict, or supplementation supplies fit. Create `subject-index-item-assessments-v5` under grading policy V3; every measured locator grade equals exactly 100 times its combined credit, but Page-reference Reliability uses only the credit ledger. Item V5 and web report V7 must expose the exact evidence summary, both axis category/score/rule triples, required fit rationale or mechanical routine explanation, evidence IDs, relevant structured defects, and `min(T,F)`. Bind result V9, projection metadata V2, migration V2 if applicable, and validation receipt V2. Keep compatibility readers for V1 locator audits and supplements plus prior V7 calculation/projection artifacts. Strict substantive precision remains public; Editorial Selectivity, caps, gates, recall, weights, density, uncertainty, and rounding remain separate and unchanged.

For V6-to-V7 score-only migration, use `dimension_score_v7_cli.py migrate-v6-to-v7` with `subject-index-v6-to-v7-migration-input-v1`; the current tool release is `dimension-score-cli-v7.1.0`. Preserve every V6 byte and recompute V6 from the exact frozen inputs before calculating V7. Historical `structure-audit-v3` defects missing `defect_kind` may use the closed code/severity compatibility rule. If otherwise valid classifiers share the exact evaluation/candidate/audit/locator/path identity but imply different categories and disagree only on derived fit, route them under `F-COMPAT-LEGACY-FIT-CONFLICT-TO-SUPPLEMENT-V1` without precedence or historical amendment; preserve a frozen `absent` treatment class in that unresolved record, while `unavailable` treatment remains uninspectable and deterministic. All broader identity, schema, artifact, prose, and contradiction failures remain invalid. `LOC_POS` remains consequence-only. Derive and report mutually exclusive deterministic/unresolved/invalid preflight groups, per-reason counts, and the unsupplemented unresolved set/hash without an aggregate score before reading any locator-fit supplement. New supplements use `subject-index-v7-locator-fit-supplement-v2` and require every-and-only coverage, exact per-view artifact/provenance hashes, category-only scoring decisions, and either public-safe rationale text or a validated hash-bound rationale-ledger reference per decision. Historical V1 supplements remain readable. Scoring consumes only the structured category; rationale is projection metadata. A conflict supplement resolves prospective V7 fit without naming a correct historical classifier. Derive display/range/atomic quantities mechanically from normalized candidate and inventory groupings; stop rather than parse display prose. A sole structured atomic-threshold false positive may be removed only from the active V7 projection when both corrected triggers are false and no independent basis remains. A newly exposed trigger is `review_required` and blocks full scoring until a narrow supplemental architecture review. Recalculate every representation-adjusted view from its own calculation, structure, locator-fit supplement when applicable, and provenance artifacts; preserve gates exactly and emit a hash-bound validation receipt. Treat all evaluation results as regression observations, never targets. Read [score-migration-v6-to-v7.md](references/score-migration-v6-to-v7.md).
