...
For V6-to-V7 score-only migration, use `dimension_score_v7_cli.py migrate-v6-to-v7` with `subject-index-v6-to-v7-migration-input-v1`. Preserve every V6 byte and recompute V6 from the exact frozen inputs before calculating V7. Derive display/range/atomic quantities mechanically from normalized candidate and inventory groupings; stop rather than parse display prose. A sole structured atomic-threshold false positive may be removed only from the active V7 projection when both corrected triggers are false and no independent basis remains. A newly exposed trigger is `review_required` and blocks full scoring until a narrow supplemental architecture review. Recalculate every representation-adjusted view from its own calculation and provenance artifacts, preserve gates exactly, and emit a hash-bound validation receipt. Do not modify or tune against an Oxford evaluation; the Oxford-shaped entry is a regression case only. Read [score-migration-v6-to-v7.md](references/score-migration-v6-to-v7.md).
